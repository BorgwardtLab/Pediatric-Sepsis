...
class ConsensusCluster:
    ...
    def _internal_resample(self, Xs, proportion):
        ...
        """
        resampled_indices = sample(range(Xs.shape[0]),  int(Xs.shape[0]*proportion))
        return resampled_indices, Xs.values[resampled_indices, :]
    ...
    def fit(self, Xs, verbose=False):
        ...
        """
        Mk = np.zeros((self.K_-self.L_, Xs.shape[0], Xs.shape[0]))
        Is = np.zeros((Xs.shape[0],)*2)
        for k in range(self.L_, self.K_):  # for each number of clusters
            ...
            for h in range(self.H_):  # resample H times
                ...
                resampled_indices, Xs_sampled = self._internal_resample(
                    Xs, self.resample_proportion_)
                Mh = self.cluster_(n_clusters=k, random_state=10, n_init=10).fit_predict(Xs_sampled)
            ...
            Mk[i_] /= Is+1e-8
            Mk[i_, range(Xs.shape[0]), range(
                Xs.shape[0])] = 1  # always with self
