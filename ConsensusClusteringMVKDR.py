...
class ConsensusCluster:
    ...
    def _internal_resample(self, Xs, proportion):
        ...
        """
        resampled_indices = sample(range(Xs[0].shape[0]),  int(Xs[0].shape[0]*proportion))
        return resampled_indices, Xs[0].values[resampled_indices, :], Xs[1].values[resampled_indices, :]
    ...
    def fit(self, Xs, verbose=False):
        ...
        for k in range(self.L_, self.K_):  # for each number of clusters
            ...
            for h in range(self.H_):  # resample H times
                ...
                resampled_indices, resample_view1,  resample_view2 = self._internal_resample(
                    Xs, self.resample_proportion_)
                
                sigma1 = math.sqrt(self._mediandist(resample_view1))
                sigma2 = math.sqrt(self._mediandist(resample_view2))
                Mh, km_obj = self.cluster_(resample_view1, resample_view2, k,
                                           sigma1, sigma2, 0.1, 0.1, 10)
            ...
            Mk[i_] /= Is+1e-8
            Mk[i_, range(Xs[0].shape[0]), range(
                Xs[0].shape[0])] = 1  # always with self
